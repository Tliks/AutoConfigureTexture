...
public class TextureGradientProcessor : IDisposable
{
    ...
    public async Task<float> CalculateIntensityAsync(Texture2D texture, Mesh mesh)
    {
        var width = texture.width;
        var height = texture.height;
        _maskTexture = GetUVMask(mesh, width, height);
        _gradientTexture = CalculateGradient(texture, _maskTexture);
        var histogramData = await CalculateHistogram(_gradientTexture);
        CalculateIntensity(histogramData, out var intensity);
        return intensity;
    }
    ...
    //
    private async Task<Dictionary<float, int>> CalculateHistogram(RenderTexture inputTexture)
    {
        ...
        var request = AsyncGPUReadback.Request(histogramBuffer, (req) =>
        {
            tcs.SetResult(req);
        });

        var asyncRequest = await tcs.Task;
        ...
        var data = asyncRequest.GetData<int>();
        var histogramData = initialData;
        histogramData = data.ToArray();
        histogramBuffer.Release();
    ...
    // threshold: 上位0.5%は勘
    private float CalculateIntensity(Dictionary<float, int> histogramData, out float targetkey, float threshold = 0.005f)
    {
        targetkey = 0f;
...
{
    public static async Task<float[]> CalculateGradientIntensityAsync(TextureInfo[] infos)
    {
        var targets = infos
            .Select(info =>
            {
        ...
        return await CalculateGradientIntensityAsync(targets);
    }
    public static async Task<float[]> CalculateGradientIntensityAsync((Texture2D, Mesh)[] targets)
    {
        var processors = new List<TextureGradientProcessor>();
        var tasks = new List<Task<float>>();

        foreach (var pair in targets)
        {
            var processor = new TextureGradientProcessor();
            processors.Add(processor);
            tasks.Add(processor.CalculateIntensityAsync(pair.Item1, pair.Item2));
        }
